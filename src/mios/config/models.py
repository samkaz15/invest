...
from mios.common.errors import InvalidIdError
from mios.common.ids import IdKind, validate_id
from mios.common.labels import Dimension
...
class EntityTaxonomy(MiosModel):
...
class RelationshipTaxonomy(MiosModel):
...
class SourceSpec(MiosModel):
    ...
    source_id: str
    name: str
    kind: Literal["rss", "http_json", "http_csv"]
    url: str
...
class ResilienceDefaults(MiosModel):
...
class PipelinesConfig(MiosModel):
...
class AssetConfig(MiosModel):
    """config/assets/<asset>.yaml — one analysed asset.

    An asset is configuration, never a code assumption: adding one must not
    require a core schema change. ``drivers`` names the macro series whose
    moves this asset is interpreted against (the cross-asset chain of
    docs/REPOSITORY_AUDIT.md §13.2); it is documentation of intent that the
    analysis layer reads, not a computation.
    """

    asset_id: str
    name: str
    symbol: str  # provider-neutral quote symbol, e.g. "XAU/USD"
    asset_class: str  # fx | commodity | equity_index | bond
    quote_currency: str = "USD"
    drivers: list[str] = Field(default_factory=list)  # series_ids, most causal first


class ScoringConfig(MiosModel):
    """config/scoring.yaml — macro dimension weights.

    Weights are configuration so that changing them is a reviewable diff and
    every score card can record which ``weights_version`` produced it.
    """

    weights_version: str
    weight_sets: dict[str, dict[str, float]]  # regime name -> dimension -> weight

    @model_validator(mode="after")
    def _weights_known_and_positive(self) -> "ScoringConfig":
        if "default" not in self.weight_sets:
            raise ValueError("weight_sets must define a 'default' set")
        known = {d.value for d in Dimension}
        for regime, weights in self.weight_sets.items():
            if not weights:
                raise ValueError(f"empty weight set: {regime!r}")
            for dim, w in weights.items():
                if dim not in known:
                    raise ValueError(f"unknown dimension {regime}.{dim!r} (known: {sorted(known)})")
                if w < 0:
                    raise ValueError(f"negative weight {regime}.{dim}={w}")
        return self
