...
from mios.audit import AgentRunRecord, AuditLogger, AuditSink
from mios.common.errors import MiosError
from mios.common.ids import IdKind, new_id
...
from mios.ingestion.rawitem import build_raw_item
...
class CollectError(MiosError):
    """A collection run failed (retryable at the job level)."""


class CollectReport(MiosRecord):
...
class Collector:
    ...
    def disabled_source_ids(self) -> list[str]:
        ...
        Reported explicitly: a source nobody collects is a data gap, and
        silent gaps are forbidden."""
        return sorted(sid for sid, spec in self._sources.items() if not spec.enabled)
