...
* **Slug** — master data humans read and write in YAML/reviews:
  ``ent_federal_reserve``, ``src_fred_cpi``, ``ser_us_cpi_yoy``.
* **Dated** — one-per-day or per-occurrence artifacts:
  ``evt_2026-09-11_us-cpi-august``, ``rel_2026-09-11_us-cpi``,
  ``fc_2026-09-12_us-cpi-2026-08``.
...
class IdKind(StrEnum):
    ...
    ENTITY = "ent"
    CHAIN = "chain"
    SERIES = "ser"  # a macro time series, e.g. ser_us_cpi_yoy
    EVENT = "evt"
...
def make_slug_id(kind: IdKind, name: str) -> str:
    """Human-readable master-data ID, e.g.
    ``make_slug_id(ENTITY, "Federal Reserve")`` → ``ent_federal_reserve``.
    Slug kinds use snake_case."""
...
def make_dated_id(kind: IdKind, day: str, slug: str) -> str:
    """Dated ID, e.g. ``evt_2026-09-11_us-cpi-august``. ``day`` is the
...
def pydantic_id_validator(value: str, kind: IdKind) -> str:
    """:func:`validate_id` for use inside pydantic validators.

    Re-raises as ``ValueError`` so a bad id surfaces as a normal
    ValidationError alongside every other schema violation. This is the one
    definition; before the MIOS cleanup it was copied into three modules
    (docs/REPOSITORY_AUDIT.md §7 D-1).
    """
    try:
