...
`.github/workflows/daily.yml` が **22:00 UTC（日本時間 翌朝7:00、平日）** に実行する。
cron は UTC で評価されるため曜日指定は `0-4`（日〜木）になる —— 日本の月〜金の朝は、
UTC では日〜木の夜だからである。

この時刻には理由がある。**米国の指標発表（08:30 ET = 12:30／13:30 UTC）と
米株の引け（20:00／21:00 UTC）の両方より後**である必要がある。そうでないと、
その日付のレポートにその日の米国の数字が入らない。
以前の 07:10 UTC はコメントに「米国の発表後」と書きながら実際には5時間前で、
この条件を満たしていなかった。
